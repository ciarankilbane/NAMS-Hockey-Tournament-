...
async function startServer() {
      ...
      if (otherSubmission) {
        if (otherSubmission.score1 === score1 && otherSubmission.score2 === score2) {
          ...
          const otherScorers = typeof otherSubmission.scorers === 'string'
            ? JSON.parse(otherSubmission.scorers)
            : otherSubmission.scorers;
      ...
      if (tournament_type === 'competitive') {
        if (teams.length >= 9) {
          await db.run(
            ...
            [teams[7].id, teams[8].id, tournament_type, 'play-off-8v9']
          );
        }
  ...
  app.delete("/api/teams/:id", async (req, res) => {
    ...
    try {
      ...
      await db.run("DELETE FROM goals WHERE match_id IN (SELECT id FROM matches WHERE team1_id = ? OR team2_id = ?)", [id, id]);
